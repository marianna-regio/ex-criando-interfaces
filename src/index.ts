...
function showRepositories(user: GitHubUser) {
  ...
  for (let i = 0; i <= 5; i++) {
    console.log(`
            ...
            É um fork: ${user.repos[i].fork ? "Sim" : "Não"}
            `);
  }
}
...
async function getUsersMostRepos() {
  // copia o array de usuários e ordena o novo com base no número de repositórios em ordem decrescente
  const sortedUsers = users.slice().sort((a, b) => b.public_repos - a.public_repos);
  // Retorna os tres primeiros usuários do array ordenado
  const topFive = sortedUsers.slice(0, 3).forEach((user,index) =>
